...
| --- | --- | --- |
| Lambda Function | `aws_lambda_function.tracker` | トラッキングピクセルのリクエスト受信・ログ出力 |
| Lambda Function URL | `aws_lambda_function_url.tracker` | 外部公開の HTTP エンドポイント（Auth: NONE） |
| CloudWatch Log Group | `aws_cloudwatch_log_group.tracker` | 開封イベントの JSON ログ保存（保持: 30日） |
...
│   ├── lambda.tf            # Lambda + Function URL + CloudWatch Log Group
│   ├── iam.tf               # IAM 実行ロール
│   └── src/
│       └── lambda_function.py
├── scripts/
...
└── html/
    └── test.html            # 動作確認用 HTML（ブラウザで開くだけでピクセル発火）
```
